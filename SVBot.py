...
class SVBot(Bot):
    """
    1. Selective Value Bot: assigns each card a cost based on rank points
    2. adds a fixed penalty to trump cards
    3. tries to win tricks using the lowest-cost winning card and otherwise discards the cheapest card.
    """
    TRUMP_PENALTY = 10  # extra cost added to trump cards
    ...
    def choose_as_leader(self, perspective, moves, trump_suit):
        """
        1. prefer cheap non-trump “good” cards (ACE/TEN/KING) when possible
        2. compare cheapest good non-trump vs cheapest trump
        3. fall back to the overall cheapest card.
        """
    ...
    def choose_as_follower(self, perspective, moves, leader_move, trump_suit):
        """
        1. collect all legal moves that beat the leader's card
        2. if any exist, play the lowest-cost winning move
        3. otherwise, discard the lowest-cost legal card.
        """
